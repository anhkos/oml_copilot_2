...
export function createDeleteInstanceToolHandler(restClient) {
    return async ({ ontologyIri, memberIri, referencingUri }) => {
        const result = await restClient.update([{ kind: "deleteMemberCascade", ontologyIri, memberIri }], referencingUri);
        if (!result.ok) {
