...
export function createServer(restClient, startupStatus) {
    ...
    server.registerTool("fuzzy_search", {
    ...
    server.registerTool("get_assertions", {
        description: "Returns the RDF assertions for OML models serialized as Turtle (or another RDF format). " +
    ...
    server.registerTool("create_concept_instance", {
        description: "Creates a new concept instance in a description ontology. " +
    ...
    server.registerTool("create_relation_instance", {
        description: "Creates a new relation instance linking source and target instances. " +
    ...
    server.registerTool("delete_instance", {
        description: "Deletes an OML instance and all of its references (cascade). Use fuzzy_search to find the memberIri first.",
        inputSchema: deleteInstanceSchema,
    }, createDeleteInstanceToolHandler(restClient));
    server.registerTool("delete_type_assertion", {
    ...
    server.registerTool("delete_property_value", {
        description: "Removes all assertions for a given property on an OML instance. Use get_assertions to confirm the current value first.",
        inputSchema: deletePropertyValueSchema,
    }, createDeletePropertyValueToolHandler(restClient));
    server.registerTool("update_property_value", {
        description: "Updates an existing property assertion on an OML instance. Use get_assertions to read the current value first.",
