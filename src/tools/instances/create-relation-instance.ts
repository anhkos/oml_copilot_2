...
type CreateRelationInstanceArgs = {
  ontologyIri: string;
  instanceName: string;
  types?: string[];
...
export function createRelationInstanceToolHandler(restClient: OmlRestClient) {
  return async ({ ontologyIri, instanceName, types, propertyValues, annotations, referencingUri }: CreateRelationInstanceArgs) => {
