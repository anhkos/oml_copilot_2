...
type UpdatePropertyValueArgs = {
  ...
  subjectIri: string;
  predicateIri: string;
  object: string | number | boolean | { value: string; langTag?: string; datatypeIri?: string; typeIri?: string };
  referencingUri?: string;
};

export function createUpdatePropertyValueToolHandler(restClient: OmlRestClient) {
  return async ({ ontologyIri, subjectIri, predicateIri, object, referencingUri }: UpdatePropertyValueArgs) => {
    const result = await restClient.update(
      [{ kind: "updateAssertion", ontologyIri, subjectIri, predicateIri, object }],
      referencingUri,
    );
    if (!result.ok) {
