...
export type StartupCheckLevel = "ok" | "warning" | "error";
...
export async function buildStartupStatus(workspaceRootInput: string): Promise<StartupStatus> {
...
export function hasStartupErrors(status: StartupStatus): boolean {
...
async function fetchJson(url: string): Promise<{ ok: true; data: unknown } | { ok: false; message: string }> {
  try {
...
async function countLocalModelMatches(workspaceRoot: string, relativePaths: string[]): Promise<number> {
...
function readFilesArray(data: unknown): Array<{ path: string }> | undefined {
  ...
  return files
    ...
    .filter((entry): entry is { path: string } => entry !== undefined);
}

function readStringField(data: unknown, fieldName: string): string | undefined {
  if (!isRecord(data)) return undefined;
...
async function findFirstOmlFile(workspaceRoot: string): Promise<string | undefined> {
